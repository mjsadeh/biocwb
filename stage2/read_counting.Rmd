...
### here we need to install the "airway" package first!
...
# this is just a long path to common separate files
sample.table <- read.csv(csv.file, row.names=1)
# we point to the BAM files - they have the pattern of RUN information on sample table and then on the end of it _subset.bam
bam.files <- file.path(dir, paste0(sample.table$Run, "_subset.bam"))
bam.files  # this then the charactor vectors that point to the bam files

# another way to point to the BAM file is if you have the directory
list.files(dir, ".bam$")
list.files(dir, ".bam$", full.names = TRUE)
gtf.file <- file.path(dir, "Homo_sapiens.GRCh37.75_subset.gtf")
# this file has the infromation about the gene that are in this subset of the chr that we are gonna look at
```
...
# a collection of biocondoctor packages to count the read can be find here
library(Rsamtools)
# we are just to spesify that we have a list of bam files using this function
bam.list <- BamFileList(bam.files)
# bam.list <- BamFileList(bam.files, yieldSize = ) if we have a big file and we have limited memory in R sessin, we can use yield to limit the number of record which are read at the time.
library(GenomicFeatures)   
# this package allow us to build transcript data base
# for Bioc 3.0 use the commented out line
# txdb <- makeTranscriptDbFromGFF(gtf.file, format="gtf")
txdb <- makeTxDbFromGFF(gtf.file, format="gtf")
# this function will build a data database which containd all the information about different genes, transcript and exons
txdb
# they are actually "RSQLite" data bases and they can be save by
# saveDb() loadDb()
exons.by.gene <- exonsBy(txdb, by="gene")
# we are going to extract bioconductor objest from this data base which is "Grangeslist". It is a list which each element is a gene and with in that elembet we have the exon for that gene.
exons.by.gene[[1]]
# we get the exon for the fisrt gene
length(exons.by.gene)
# the numeber of gene in our subset
summary(exons.by.gene)
